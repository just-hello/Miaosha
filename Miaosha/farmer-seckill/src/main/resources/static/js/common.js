...
function showHintMsg(title, msg){
	top.window.$.messager.alert({
		title: title,
		msg: '<div class="messager-icon messager-warning"></div><div style="padding-top: 10px;">' + msg + "</div>",
	});
}
...
$(window).ajaxSuccess(function(evt, request, settings){
	...
	if(s && s.indexOf('{"result":"fail"') != -1) {
		showHintMsg("提示",s.substring(24, s.length - 2));
	}
});
